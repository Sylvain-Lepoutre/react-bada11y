...
const LevelAAFormSchema = object({
...
type LevelAAForm = Input<typeof LevelAAFormSchema>;

export const AAForm = () => {
  const [levelAAForm, { Form, Field }] = useForm<LevelAAForm>({
    validate: valiForm(LevelAAFormSchema),
  });

  const handleSubmit: SubmitHandler<LevelAAForm> = (values) => {
    console.log(values);
  };
  ...
  return (
    <div>
      <Form
        className="mx-auto my-4 w-3/4 rounded-xl bg-gray-500 p-4 text-white"
      ...
      >
        ...
        <div className="m-4 flex flex-col">
          <label htmlFor="email">
            Email (doit être au format adresse@mail.fr)
          </label>
      ...
      <div>
        <a href="/components/AAFormDetails">En savoir + sur le formulaire</a>
      </div>
    </div>
  );
};
